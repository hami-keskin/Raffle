...
contract PayPool is ReentrancyGuard {
    ...
    event DepositApproved(uint256 indexed index);
    event DepositRejected(uint256 indexed index);
    ...
    function approveDeposit(uint256 index) external isOwner {
        ...
        DepositRecord storage record = depositHistory[index];
        require(record.status == DepositStatus.Pending, "Deposit already processed");

        record.status = DepositStatus.Approved;
